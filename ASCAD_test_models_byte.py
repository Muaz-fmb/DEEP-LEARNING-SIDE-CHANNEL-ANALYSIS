...
def rank(predictions, metadata, real_key, min_trace_idx, max_trace_idx, last_key_bytes_proba, target_byte, simulated_key):
    ...
    for p in range(0, max_trace_idx-min_trace_idx):
        # Go back from the class to the key byte. 'target_byte' is the index of the byte of interest.
        plaintext = metadata[min_trace_idx + p]['plaintext'][target_byte]
        key = metadata[min_trace_idx + p]['key'][target_byte]
        
        # NOTE: the original code had '2' hardcoded in comments, but we are using target_byte now.
...
def full_ranks(predictions, dataset, metadata, min_trace_idx, max_trace_idx, rank_step, target_byte, simulated_key):
    ...
    # Real key byte value that we will use. 'target_byte' is the index of the byte of interest.
    if (simulated_key!=1):
...
if __name__ == "__main__":
    ...
    if len(sys.argv)!=2:
        ...
        # Point to the model we trained for Byte 1 (Index 1) using the NEW dataset
        model_file="ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/cnn_best_byte%d_win1_4_desync0_epochs100_batchsize50.h5"  % target_byte
        
        # Use the NEW dataset for testing.  my_cnn_best_byte1_desync0_epochs200_batchsize200.h5
        ...
        multilabel=0
        simulated_key=0
        save_file = "test_plot_byte%d_win1_4_100epochs_2000traces.png" % target_byte
    else:
