...
# It is as simple as the computation of the result of Sbox(p[1] + k[1]) (see the White Paper)
...
def labelize(plaintexts, keys):
	return np.uint8(AES_Sbox[plaintexts[:, 9] ^ keys[:, 9]])
...
if __name__ == "__main__":
	if len(sys.argv)!=2:
		...
		# Original raw traces setup
		ascad_databases_folder_orig = ascad_data_folder + "ASCAD_databases/"
		original_raw_traces_file = ascad_databases_folder_orig + "ATMega8515_raw_traces.h5"

		# Output folder for Byte 2 (Index 1)
		ascad_databases_folder = ascad_data_folder + "ASCAD_databases_byte9_win1_4/"
		if not os.path.exists(ascad_databases_folder):
			os.makedirs(ascad_databases_folder)

		profiling_index = [n for n in range(0, 50000)]
		attack_index = [n for n in range(50000, 60000)]
		target_points=[n for n in range(39396, 39896)]
		# target_points=[n for n in range(24741, 25441)]

		profiling_desync=0
		attack_desync=0
		extract_traces(original_raw_traces_file, ascad_databases_folder + "ASCAD_byte9.h5"		  , profiling_index, attack_index, target_points, profiling_desync=0, attack_desync=0)
		extract_traces(original_raw_traces_file, ascad_databases_folder + "ASCAD_byte9_desync50.h5" , profiling_index, attack_index, target_points, profiling_desync=0, attack_desync = 50)
		extract_traces(original_raw_traces_file, ascad_databases_folder + "ASCAD_byte9_desync100.h5", profiling_index, attack_index, target_points, profiling_desync=0, attack_desync = 100)
