...
def full_ranks(predictions, dataset, metadata, min_trace_idx, max_trace_idx, rank_step, target_byte, simulated_key):
    ...
    for t, i in zip(index, range(0, len(index))):
        # Shift the prediction index so it stays in bounds of our sliced arrays
        pred_start = (t - rank_step) - min_trace_idx
        pred_end = t - min_trace_idx
        
        real_key_rank, key_bytes_proba = rank(predictions[pred_start:pred_end], metadata, real_key, t-rank_step, t, key_bytes_proba, target_byte, simulated_key)
...
def check_model(model_file, ascad_database, min_trace_idx=2000, max_trace_idx=4000, target_byte=0, multilabel=0, simulated_key=0, save_file=""):
    check_file_exists(model_file)
    ...
    # SLICE THE DATA ACCORDING TO OUR MIN AND MAX INDEX
    if len(input_layer_shape) == 2:
        input_data = X_attack[min_trace_idx:max_trace_idx, :]
    ...
    if (multilabel!=0):
        ...
        for target_byte in range(16):
            # UPDATE: Pass min_trace_idx and max_trace_idx
            ranks_i = full_ranks(predictions_sbox[target_byte], X_attack, Metadata_attack, min_trace_idx, max_trace_idx, 10, target_byte,  simulated_key)
        ...
        # UPDATE: Pass min_trace_idx and max_trace_idx
        ranks = full_ranks(predictions_sbox_i, X_attack, Metadata_attack, min_trace_idx, max_trace_idx, 10, target_byte, simulated_key)
...
def read_parameters_from_file(param_filename):
    ...
    min_trace_idx = 2000
    if ("min_trace_idx" in my_parameters):
        min_trace_idx = my_parameters["min_trace_idx"]
        
    max_trace_idx = 4000
    if ("max_trace_idx" in my_parameters):
        max_trace_idx = my_parameters["max_trace_idx"]
    ...
    return model_file, ascad_database, min_trace_idx, max_trace_idx, target_byte, multilabel, simulated_key, save_file
...
if __name__ == "__main__":
    ...
    if len(sys.argv)!=2:
        model_file="ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_trained_models/cnn_best_byte%d_win1_desync0_epochs50_batchsize50.h5"  % target_byte
        ascad_database="ATMEGA_AES_v1/ATM_AES_v1_fixed_key/ASCAD_data/ASCAD_databases_byte%d_win1/ASCAD_byte%d.h5" % (target_byte, target_byte)
        
        # DEFINED NEW VARIABLES HERE
        min_trace_idx = 2000
        max_trace_idx = 2150
        
        multilabel=0
        simulated_key=0
        save_file = "test_plot_byte%d_win1_50epochs_150traces_sliced.png" % target_byte
    else:
        model_file, ascad_database, min_trace_idx, max_trace_idx, target_byte, multilabel, simulated_key, save_file  = read_parameters_from_file(sys.argv[1])
        print("Note: Running with user provided parameters.")

    check_model(model_file, ascad_database, min_trace_idx, max_trace_idx, target_byte, multilabel, simulated_key, save_file)
